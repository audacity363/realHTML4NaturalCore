...
int rh4n_natcaller_callNatural(RH4nProperties *props) {
    ...
    if((nniret = nnifuncs->pf_nni_initialize(nnifuncs, props->natparms, NULL, NULL)) != NNI_RC_OK) {
        ...
        rh4n_natcaller_logInternalError(props, "Could not init natural session. Please check natural parameters");
        return(RH4N_RET_INTERNAL_ERR);
    }
    ...
    if((nniret = nnifuncs->pf_nni_callnat(nnifuncs, props->natprogram, 2, naturalparms, &naturalex)) != NNI_RC_OK) {
        rh4n_natcaller_handleNaturalError(props, nniret, naturalex);
